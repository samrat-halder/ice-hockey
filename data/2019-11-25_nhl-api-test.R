...
load("2019-11-25_nhl-cleaned-data.RData")
...
for (sel_season in seasons_to_pull){
  for (i in game_nums){
...
game_plays_2017_18 <- rbindlist(game_plays_l, use.names=T, idcol="game.id", fill=T)
game_plays_players_2017_18 <- rbindlist(game_plays_players_l, use.names=T, idcol="game.id", fill=T)
game_periods_2017_18 <- rbindlist(game_periods_l, use.names=T, idcol="game.id", fill=T)
game_shootout_status_2017_18 <- data.table(game.id=names(game_shootout_status_v)
                                        ,shootout=unlist(game_shootout_status_v))
game_teams_stats_2017_18 <- rbindlist(game_teams_stats_l, use.names=T, idcol="game.id", fill=T)
game_skater_stats_2017_18 <- rbindlist(game_skater_stats_l, use.names=T, idcol="game.id", fill=T)
game_goalie_stats_2017_18 <- rbindlist(game_goalie_stats_l, use.names=T, idcol="game.id", fill=T)
game_info_2017_18 <- rbindlist(game_info_l, use.names=T, idcol="game.id", fill=T)
no_play_data_2017_18 <- data.table(game.id=names(no_play_data_l)
...
game_plays_2017_18 # Play level data
game_plays_players_2017_18 # Players involved in plays
game_periods_2017_18 # Summary of periods in each game
game_shootout_status_2017_18 # Whether there was a shootout or not
game_teams_stats_2017_18 # Overall team stats
game_skater_stats_2017_18 # Player stats for each team
game_goalie_stats_2017_18 # Goalie stats for each team
game_info_2017_18 # Some game information
no_play_data_2017_18 # Tells which games did not have play data available

"Other info"
roster_list # List of data.tables with players on each team in 20172018 season
...
vF_team_rosters_2017 <- rbindlist(roster_list,idcol = 'team.id')
...
vF_game_plays_2017_18 <- merge(game_plays_2017_18,vF_play_types[,c(1,3)],by.x='result.eventTypeId',by.y='play.id')[important==1,..col_sel]
vF_game_plays_2017_18 <- type_convert(vF_game_plays_2017_18,col_types = cols(about.periodTime=col_time("%M:%S")))
vF_game_plays_2017_18[is.na(s.x), r.x := abs(coordinates.x)]
vF_game_plays_2017_18[is.na(s.x), l.x := -r.x]

vF_game_plays_players_2017_18 <- game_plays_players_2017_18[,.(game.id,eventIdx,playerType,player.id=str_conv(player.id,'UTF8'))]

col_ex <- c('home.rinkSide','away.rinkSide')
col_sel <- setdiff(names(game_periods_2017_18),col_ex)
vF_game_periods_2017_18 <- type_convert(game_periods_2017_18[,..col_sel])

vF_game_teams_stats_2017_18 <- type_convert(game_teams_stats_2017_18,col_types = cols(game.id=col_character()))

vF_game_skater_stats_2017_18 <- game_skater_stats_2017_18
col_sel <- str_subset(names(vF_game_skater_stats_2017_18),'OnIce')
vF_game_skater_stats_2017_18[, (col_sel) := lapply(.SD, function(x){ifelse(str_length(x)==4,str_c('0',x),x)}), .SDcols=col_sel]
vF_game_skater_stats_2017_18 <- type_convert(vF_game_skater_stats_2017_18, col_types = cols(
...
vF_game_goalie_stats_2017_18 <- type_convert(game_goalie_stats_2017_18, col_types = cols(
  game.id = col_character(),
  HoA = col_character(),
  player.id = col_character(),
...
vF_game_info_2017_18 <- type_convert(game_info_2017_18[,..col_sel])

### No changes
vF_game_types <- game_types
vF_stat_types <- stat_types
vF_seasons_DT <- seasons_DT
vF_all_player_ids_2017 <- all_player_ids
vF_game_shootout_status_2017_18 <- game_shootout_status_2017_18
vF_no_play_data_2017_18 <- no_play_data_2017_18
...
vF_all_player_ids_2017 # Vector of player ids extracted from the roster list

"Matching the relational database from that person on Kaggle"
vF_teams_DT # Data.table of teams
vF_game_plays_2017_18 # Play level data
vF_game_plays_players_2017_18 # Players involved in plays
vF_game_periods_2017_18 # Summary of periods in each game
vF_game_shootout_status_2017_18 # Whether there was a shootout or not
vF_game_teams_stats_2017_18 # Overall team stats
vF_game_skater_stats_2017_18 # Player stats for each team
vF_game_goalie_stats_2017_18 # Goalie stats for each team
vF_game_info_2017_18 # Some game information
vF_no_play_data_2017_18 # Tells which games did not have play data available

"Other info"
vF_player_info # Information on every player (like age, nationality, etc.)
vF_team_rosters_2017 # List of data.tables with players on each team in 20172018 season
