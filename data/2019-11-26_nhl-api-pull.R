...
load("2019-11-26_nhl-cleaned-data.RData")
save.image("2019-11-26_nhl-cleaned-data.RData")
...
seasons_to_pull_both_years <-str_c(seasons_to_pull,seasons_to_pull+1)

### Team rosters
team_seasons <- data.table(full.season= rep(seasons_to_pull_both_years,each=length(team_ids_test))
                        ,team.id= rep(vF_teams_DT$team.id,length(seasons_to_pull_both_years))
                        ,season= rep(seasons_to_pull_year,each=length(team_ids_test)))
...
vF_game_plays[is.na(s.x), r.x := abs(coordinates.x)]
vF_game_plays[is.na(s.x), l.x := -r.x]
...
sel <- str_subset(ls(),"_2017",negate = F)
rm(list=sel)
